/**
 * TVK Kasipalayam (Tamilaga Vettri Kazhagam) - Official Website Service Worker
 * Website: https://tvkkasipalayam.in
 * 
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then((cache) => {
        console.log('[TVK SW] Caching critical resources for tvkkasipalayam.in');
...
self.addEventListener('push', (event) => {
  if (event.data) {
    ...
    const options = {
      ...
      vibrate: [100, 50, 100],
      data: {
        url: data.url || 'https://tvkkasipalayam.in/'
      }
    };
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    clients.openWindow(event.notification.data.url || 'https://tvkkasipalayam.in/')
  );
});
